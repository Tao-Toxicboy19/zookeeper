...
export class AuthService implements OnModuleInit {
    private readonly logger = new Logger(AuthService.name)
    private readonly otpMailQueue: string = 'otp_mail_queue'
    private readonly resetPasswordQueue: string = 'reset_password_queue'
    ...
    async validateUser(dto: ValidateDto): Promise<UserResponse> {
        ...
        throw new GrpcNotFoundException('Invalid username or password')
    }
    ...
    async signup(dto: UserDto): Promise<EmailResponse> {
        ...
        if (userInRedis) {
            throw new GrpcAlreadyExistsException('Username already exists in cache.');
        }

        const _id: Types.ObjectId = new Types.ObjectId();
        const hashedPassword = await bcrypt.hash(dto.password, 12);
        const userCache: string = JSON.stringify({ ...dto, _id, password: hashedPassword });
    ...
    async confirmOTP(dto: ConfirmOTPDto): Promise<TokenResponse> {
